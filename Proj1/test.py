...
deep.load_state_dict(torch.load('data/pickles/deep/best_model.pt'))
print("Training DeepModel...")
acc_losses, train_accuracy, test_accuracy, train_error, test_error = helpers.train_report(deep, train_input, train_target, test_input, test_target, mini_batch_size_deep)
...
nb_epoch_deep_ws = 25
lr_deep_ws = 0.001
mini_batch_size_deep_ws = 1
nb_hidden_deep_ws = 100
...
deep_ws.load_state_dict(torch.load('data/pickles/deep_ws/best_model.pt'))
print("Training DeepModelWS...")
acc_losses, train_accuracy, test_accuracy, train_error, test_error = helpers.train_report(deep_ws, train_input, train_target, test_input, test_target, mini_batch_size_deep_ws)
...
nb_epoch_shallow = 25
lr_shallow = 0.001
mini_batch_size_shallow = 1
nb_hidden_shallow = 100
auxiliary_loss_weight_shallow = 0.5
print("Loading pickled ShallowModel...")
shallow = ShallowModel(nb_hidden_shallow, nb_epoch_shallow, lr_shallow, mini_batch_size_shallow, auxiliary_loss_weight_shallow).to(device)
shallow.load_state_dict(torch.load('data/pickles/shallow/best_model.pt'))
print("Training ShallowModel...")
acc_losses, train_accuracy, test_accuracy, train_error, test_error = helpers.train_report(shallow, train_input, train_target, test_input, test_target, mini_batch_size_shallow)
...
print("optimizer = {0}, nb_epoch = {1}, lr = {2}, mini_batch_size = {3}, nb_hidden = {4}, auxiliary_loss_weight = {5}".format(optimizer_shallow, nb_epoch_shallow, lr_shallow, mini_batch_size_shallow, nb_hidden_shallow, auxiliary_loss_weight_shallow))